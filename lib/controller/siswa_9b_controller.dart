...
class Siswa9BController extends GetxController{
  var isLoading = true.obs;
  var siswa_9b = <Siswa9BData>[].obs;

  loadData({bool withLoading = false}) async {
    try {
      isLoading.value = true;
      var response = await apiConfig.get(UrlConfig.baseUrl()+'kelas9b');
      //debugPrint('response notes : $response');
      if(response!='error' && response!='fatal'){
        siswa_9b.value = Siswa9BModel.fromJson(jsonDecode(response.toString())).data!;
