...
import 'package:aplikasi_si/model/model_civitas_guru.dart';
...
class GuruController extends GetxController{
  ...
  var guru = <GuruData>[].obs;

  loadData({bool withLoading = false}) async {
    try {
      isLoading.value = true;
      var response = await apiConfig.get(UrlConfig.baseUrl()+'guru');
      //debugPrint('response notes : $response');
      if(response!='error' && response!='fatal'){
        guru.value = GuruModel.fromJson(jsonDecode(response.toString())).data!;
        isLoading.value=false;
      }else{
        isLoading.value=false;
      }
      print('ini ' + guru.length.toString());
      print(UrlConfig.baseUrl()+'guru');
    } catch (e) {
