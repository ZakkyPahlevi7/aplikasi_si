...
import 'package:aplikasi_si/model/model_prestasi.dart';
...
class KepalaController extends GetxController{
  ...
  var prestasi = <PrestasiData>[].obs;

  loadData({bool withLoading = false}) async {
    try {
      isLoading.value = true;
      var response = await apiConfig.get(UrlConfig.baseUrl()+'prestasi');
      //debugPrint('response notes : $response');
      if(response!='error' && response!='fatal'){
        prestasi.value = PrestasiModel.fromJson(jsonDecode(response.toString())).data!;
